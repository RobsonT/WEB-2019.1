...
public class CarrinhoController {
	...
	public ModelAndView adicionarPrato(@PathVariable Long id, HttpSession session) {
		Prato prato = pratoService.buscarPorId(id);
		session.setAttribute("prato", prato);
		ModelAndView mv = new ModelAndView("AdicionarPrato");
	...
	public ModelAndView salvarItem(int quantidade, HttpSession session) {
		...
		if(session.getAttribute("itens") != null) {
			...
			for(Item i: itens) {
				if(i.getPrato().getId() == prato.getId()) {
	...
	@RequestMapping("/removerItem/{id}")
	public ModelAndView removerItem(@PathVariable Long id, HttpSession session) {
		List<Item> itens = new ArrayList<Item>();
		if(session.getAttribute("itens") != null) {
			itens = (List<Item>) session.getAttribute("itens");
			for(int i = 0; i < itens.size(); i++) {
				if(itens.get(i).getPrato().getId() == id) {
					itens.remove(i);
				}
			}
		}
	...
	public ModelAndView carrinho(HttpSession session) {
		ModelAndView mv = new ModelAndView("ListarItem");
//		System.out.println(carrinhoService.getItens().get(0));
		...
		if((List<Item>) session.getAttribute("itens") != null) {
			...
			for (Item item: (List<Item>) session.getAttribute("itens")) {
				valor += item.getPrato().getValor() * item.getQuantidade();
			}
		}else {
